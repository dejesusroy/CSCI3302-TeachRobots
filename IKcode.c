...
short *reconstruct_path(short *prev, int source_vertex, int dest_vertex)
{
  ...
  /*********************************************/
  //  We have to change goal_changed to FALSE  //
  //  in order to continue to the next state.  //
  //  We also need to make sure the path       //
  // contains the initial vertex in its path   //
  /*********************************************/
...
void loop ()
{
  ...
  switch(machine_state)
  {
    ...
    //Once computer side decided grid id for the object, it should send the id through bluetooth port
    case STATE_WAIT_FOR_COMMAND:
      if(Serial1.available())
      {
        int inputString = Serial1.read();
        goal_index += (char)inputString;
      }
    ...
    case STATE_START:
      /*****************************************/
      // This state will compute all errors    //
      // and create our path. These functions  //
    ...
    case STATE_HAS_PATH:
      /*****************************************/
      // This state will assign a goal pose    //
      // with respect to the next waypoint on  //
      // the final path array. It will 
      /*****************************************/
      if (goal_changed)
      {
        machine_state = STATE_START;
